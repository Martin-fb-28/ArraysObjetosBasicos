    ...
    public static void main(String[] args){
        ...
        for(int i = 0; i< arrayProductos.length; i++){
            if(arrayProductos[i].getPrecio() > valorRandom){
                System.out.println("\nEl/los producto/os que superan el valor random, " + valorRandom + ", es/son: " + arrayProductos[i].getNombre());
            }
        }

        for(int i = 0; i< arrayProductos.length; i++){
            if(arrayProductos[i].getPrecio() == valormasAlto){
                System.out.println("\nEl producto más caro es: " + arrayProductos[i].getNombre() + ", y tiene este precio: " + arrayProductos[i].getPrecio());
            }
        }
    }
}
